...
## parameterization
mode = 3
# mode=0: sequence data making and normalization #mode=1: AE model training #mode2=: AE model prediction #mode=3: AE missing data prediction using iteration method
seq = 20
Nan_Number = 0
...
# validation data plot
    ...
    ###matrix
    column_name = ["Perason CC", "Sperman CC", "Spearman p-val", "RMSE", "R2", "KS D", "KS p-val"] * (
                well_feature_num - 1)
    ...
    for i in range(0, well_feature_num - 1):
        ...
        testing_nan_dts_data[0, 0] = np.round(
            np.corrcoef(moving_averages_pred_nan[:, 4], org_x_test_data_seq[:, 4])[0, 1],
            2)  ## initial CC
        testing_nan_dts_data[0, 1] = np.round(
            np.sqrt(mean_squared_error(load_MinMaxScaler_scaler.inverse_transform(moving_averages_pred_nan)[:, 4],
                                       load_MinMaxScaler_scaler.inverse_transform(org_x_test_data_seq)[:, 4])),
            2)  ##RMSE
        ...
        for m in range(1, iteration):
            ...
            for s in range(pred_test_NaN_seq.shape[1]):
                moving_averages_pred_nan[:, s] = np.convolve(pred_test_NaN_seq[:, s],
                                                             np.ones(window_size) / window_size, mode='valid')
            org_x_test_data_seq = testing_data_seq[front_idx:-back_idx, :]
            testing_nan_dts_data[m, 0] = np.round(
                np.corrcoef(moving_averages_pred_nan[:, 4], org_x_test_data_seq[:, 4])[0, 1],
                2)  ##CC
        ...
        result_path = save_path_model + 'testing_welllog_NaN_' + title[k] + '_Well num_' + str(
            test_well_num) + '_AE_' + str(seq) + '_pred_iteration_subtract.xlsx'
        ...
        path_savefig_testing_litho = path_savefig + 'testing_litho_NaN_' + title[k] + '_Well num_' + str(
            test_well_num) + '_AE_model_subtraction_iteration_' + str(m) + '_seq_' + str(seq) + '.jpg'
        ...
        pred_x_test_data_new = pred_test_NaN.reshape(testing_data.shape[0], testing_data.shape[1],
                                                     testing_data.shape[2])
        ...
        path_savefig_testing_nan = path_savefig + 'testing_welllog_NaN_' + title[k] + '_Well num_' + str(
            test_well_num) + '_AE_model_seq_' + str(seq) + '_iter.jpg'
        ...
        np.save(save_path_model + 'testing_welllog_NaN_' + title[k] + '_Well num_' + str(
            test_well_num) + '_AE_model_seq_' + str(seq) + '_pred_iter.npy',
                pred_x_test_data_new_mean)

        np.save(save_path_model + 'testing_welllog_NaN_' + title[k] + '_Well num_' + str(
            test_well_num) + '_AE_model_seq_' + str(seq) + '_pred_test_subtract.npy', pred_test_subtract_previous)
        np.save(save_path_model + 'testing_welllog_NaN_' + title[k] + '_Well num_' + str(
            test_well_num) + '_AE_model_seq_' + str(seq) + '_testing_curve_CC_RMSE.npy',
    ...
    result_path = save_path_model + 'testing_welllog_NaN_Well num_' + str(
        test_well_num) + '_AE_model_seq_' + 'DTSandAnother_missing_' + str(seq) + '_pred_result_excel_iter.xlsx'
